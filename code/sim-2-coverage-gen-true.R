...
for (i in seq_along(fname_sim_2)) {
  ...
  tryCatch({
    ...
    for (p in c(0.01, 0.05)) {
      ...
      v.ind <- var_cc_ind(N.main, al.vec, Pi.mat, p, beta)
      v.ego <- var_cc_ego(N.main, al.vec, Pi.mat, p, beta)
...
kable(
  FINAL_TABLE,
  caption = "Unified coverage and interval length results (Sim-2, true parameters, beta = 1/2)"
)

write.table(
  FINAL_TABLE,
  "sim2_true_parameter_results.txt",
  row.names = FALSE,
  quote = FALSE,
  sep = "\t"
)