...
for (i in seq_along(fname_sim_1)) {
  ...
  tryCatch({
    ...
    for (p in c(0.01, 0.05)) {
      ...
      v.ind <- var_cc_ind(N.main, cl, pi, p, 0)
      v.ego <- var_cc_ego(N.main, cl, pi, p, 0)
      
      f <- (p + (1 - p) * p^2) / (3 * p^2 * (1 - p) + p^3)
      
      ## Induced CI (unchanged)
      up.ind <- est.ind_cc + 1.96 * sqrt(v.ind)
      lo.ind <- est.ind_cc - 1.96 * sqrt(v.ind)
      
      ## Ego CI (bias-corrected, unchanged)
...
write.table(
  ...
  "sim1_iteration_runtime_log.txt",
  row.names = FALSE,
  quote = FALSE,
  sep = "\t"
)
...
kable(
  FINAL_TABLE,
  caption = "Unified coverage and interval length results (Sim-1, all statistics, parameter unknown)"
)

write.table(
  FINAL_TABLE,
  "sim1_all_statistics_results-est.txt",
  row.names = FALSE,
  quote = FALSE,
  sep = "\t"
)
